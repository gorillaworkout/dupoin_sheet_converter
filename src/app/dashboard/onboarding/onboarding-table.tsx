...
import { Search, Filter, Plus, LogIn, Clock, CheckCircle2 } from "lucide-react";
...
function getChecklistProgress(record: OnboardingRecord) {
  let completed = 0;
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (n === "new hire" || n === "transfer" || n === "rehire")
...
export function OnboardingTable({ records }: OnboardingTableProps) {
  ...
  const completedCount = records.filter((r) => isDone(r.completed)).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-xs text-zinc-500">
        Menampilkan {filtered.length} dari {records.length} onboarding
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-white/5 hover:bg-transparent">
              ...
              <TableHead className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Departemen</TableHead>
              <TableHead className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Posisi</TableHead>
              <TableHead className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Tanggal Mulai</TableHead>
              ...
              <TableRow>
                <TableCell colSpan={6} className="h-32 text-center text-sm text-zinc-500">
                  {search || statusFilter !== "all"
                    ? "No onboarding records match the filter."
              ...
              filtered.map((rec) => {
                ...
                return (
                  <TableRow key={rec.record_id || rec.id} className="border-white/5 transition-colors hover:bg-white/[0.03]">
                    <TableCell>
                      <Link
                        href={`/dashboard/onboarding/${rec.record_id || rec.id}`}
                        className="font-medium text-white hover:text-cyan-400 transition-colors"
                      >
                        {rec.fullName || "—"}
                      </Link>
                    </TableCell>
                    <TableCell className="text-zinc-300">{rec.department || "—"}</TableCell>
                    <TableCell className="text-zinc-300">{rec.position || "—"}</TableCell>
                    <TableCell className="text-zinc-400">{rec.commencementDate || "—"}</TableCell>
                    <TableCell><StatusBadge status={rec.completed || rec.incomingStatus} /></TableCell>
                    <TableCell>
