...
import { Search, Users, UserCheck, Building2, Filter } from "lucide-react";
  ...
  const filtered = useMemo(() => {
    return employees.filter((emp) => {
      ...
      const matchesCompany =
        companyFilter === "all" || emp.company === companyFilter;
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((emp) => (
                ...
                >
                  <TableCell>
                    <Link
                      href={`/dashboard/employees/${emp.record_id}`}
                  ...
                  <TableCell className="text-zinc-400 text-xs">
                    {emp.work_email || emp.business_email || "—"}
                  </TableCell>
                </TableRow>
              ))
            )}
